...
const TabMenu = ({ category }) => {
   ...
   const getTotalCount = async () => {
      const { data } = await axios.get(`${import.meta.env.VITE_URL}/totalMenuCount/${value}`);
      ...
      <Box sx={{ width: '100%' }}>
         ...
         <div className='max-w-7xl mx-auto'>
            ...
            <div className='mt-10 flex justify-center items-center gap-4'>
               ...
               {/* Previus Button */}
               <button disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)} className="btn btn-circle bg-[#D1A054] border-2 border-[#414143] hover:bg-[#D8DADC] hover:border-[#D8DADC] disabled:border-[#D8DADC]">
                  <MdArrowBack className='text-xl text-black' />
               </button>
               <div className='space-x-4'>
                  {
                     buttonCount.map(button => <button key={button} onClick={() => setCurrentPage(button)} className={`btn btn-sm btn-circle text-shadow-black ${currentPage === button ? "bg-[#008000]" : ""}`}>{button}</button>)
                  }
               </div>
               {/* Next Button */}
               <button disabled={currentPage === totalPage} onClick={() => setCurrentPage(currentPage + 1)} className="btn btn-circle bg-[#D1A054] border-2 border-[#414143] hover:bg-[#D8DADC] hover:border-[#D8DADC] disabled:border-[#D8DADC]">
